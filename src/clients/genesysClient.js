...
export class GenesysClient {
  ...
  async sendInboundMessage(messagePayload) {
    const query = this.#config.prefetchConversationId ? '?prefetchConversationId=true' : '';
    return this.#request(`/api/v2/conversations/messages/${encodeURIComponent(this.#config.integrationId)}/inbound/open/message${query}`, {
      method: 'POST',
      body: messagePayload
    });
  }
  ...
  async sendInboundReceipt(receiptPayload) {
    return this.#request(`/api/v2/conversations/messages/${encodeURIComponent(this.#config.integrationId)}/inbound/open/receipt`, {
      method: 'POST',
      body: receiptPayload
    });
  }
  ...
  async #getAccessToken() {
    ...
    if (!responseBody?.access_token) {
      throw new HttpIntegrationError('Genesys OAuth response does not contain access_token.', {
        status: response.status,
        body: responseBody
      });
    }
