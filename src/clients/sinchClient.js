...
export class SinchClient {
  ...
  async sendMessage(messagePayload) {
    return this.#request(`/v1/projects/${encodeURIComponent(this.#config.projectId)}/messages:send`, {
      method: 'POST',
      body: messagePayload
    });
  }
  ...
  async #getAccessToken() {
    ...
    const basicAuth = Buffer.from(`${this.#config.keyId}:${this.#config.keySecret}`).toString('base64');
    ...
    if (!responseBody?.access_token) {
      throw new HttpIntegrationError('Sinch OAuth response does not contain access_token.', {
        status: response.status,
        body: responseBody
      });
    }
